// Данные рецептов (краткая версия для категорий)
const recipes = [
    {
        id: 1,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (category) {
        ...
        title = categoryTitles[category] || "Рецепты";
    }
    
    if (season) {
        // Здесь можно добавить фильтрацию по сезону
        const seasonTitles = {
            'spring': 'Весенние рецепты',
            'summer': 'Летние рецепты',
            'autumn': 'Осенние рецепты',
            'winter': 'Зимние рецепты',
            'holiday': 'Праздничные блюда'
        };
        
        title = seasonTitles[season] || "Сезонные рецепты";
    }
    ...
    if (recipesContainer) {
        ...
        filteredRecipes.forEach(recipe => {
            ...
            recipeCard.innerHTML = `
                ${stampHtml}
                <button class="save-recipe-btn" aria-label="Сохранить рецепт">
        ...
        // Инициализация кнопок сохранения для загруженных рецептов
        initSaveButtons();
    }
});